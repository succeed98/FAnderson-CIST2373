 ...
 */
@WebServlet(urlPatterns = {"/PatientHomeServlet"})
public class PatientHomeServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("--->PatientHomeServlet was called <---");
        response.setContentType("text/html;charset=UTF-8");
        try (PrintWriter out = response.getWriter()) {
            String patId, firstName, lastName, email, address, insCo;
            patId = request.getParameter("patId");
            firstName =  request.getParameter("firstName");
            lastName = request.getParameter("lastName");
            email = request.getParameter("email");
            address = request.getParameter("address");
            insCo = request.getParameter("insCo");
            
            System.out.println(patId + " " + firstName + " " + lastName + " "+ email + " "  + address + " " + insCo);
